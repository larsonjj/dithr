...
function buildLevel(): void {
    ...
    // Coins
    for (let x = 9; x <= 11; ++x) { setTile(x, floor - 5, T_COIN); score--; }
    for (let x = 19; x <= 21; ++x) { setTile(x, floor - 6, T_COIN); score--; }
    for (let x = 31; x <= 34; ++x) { setTile(x, floor - 5, T_COIN); score--; }
    score = 0; // reset — we just needed to count
}
