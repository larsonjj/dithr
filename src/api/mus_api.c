...
{
    (void)this_val;
    dtr_mus_stop(AUD(ctx), dtr_api_opt_int(ctx, argc, argv, 0, 0));
    return JS_UNDEFINED;
}
...
{
    (void)this_val;
    dtr_mus_volume(AUD(ctx), (float)dtr_api_opt_float(ctx, argc, argv, 0, 1.0));
    return JS_UNDEFINED;
}
...
static JSValue js_mus_getVolume(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv)
{
    ...
    (void)argc;
    (void)argv;
    return JS_NewFloat64(ctx, dtr_mus_get_volume(AUD(ctx)));
}
