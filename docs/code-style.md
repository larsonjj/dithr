...
Based on the [c-code-style](https://github.com/MaJerle/c-code-style) repo
...
The keywords _MUST_, _MUST NOT_, _REQUIRED_, _SHALL_, _SHALL NOT_, _SHOULD_, _SHOULD NOT_, _RECOMMENDED_, _NOT RECOMMENDED_, _MAY_, and
_OPTIONAL_ in this document are to be interpreted as described in BCP 14 [RFC2119] [RFC8174]
...
- Use `stdbool.h` library for `true` or `false` values
...
- Always use `// comment` for single-line comment
...
- Make comments `4-spaces` aligned (examples below) to next available indent
...
my_func(void) {
    ...
    a = call_func_returning_char_a(a);    // This is comment with 4 spaces indent from beginning of line
}
```
...
**Option 1:** When structure is declared with _name only_, it _MUST not_ contain `_t` suffix after its name.
...
**Option 2:** When structure is declared with _typedef only_, it _has to_ contain `_t` suffix after its name.
...
**Option 3:** When structure is declared with _name and typedef_, it _MUST NOT_ contain `_t` for basic name and it _MUST_ contain `_t` suffix after its name for typedef part.
...
- Documentation for functions MUST be written in function implementation (source file usually) or for macros in header file
